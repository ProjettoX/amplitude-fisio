...
const GlobalStyle = () => (
  <style>{`
    ...
    *{box-sizing:border-box;}
    ...
    .ficha-tabs{ display:flex; gap:6px; flex-wrap:wrap; margin-bottom:16px; }
    .ficha-tab{
      font-family:'JetBrains Mono',monospace; font-size:10px; letter-spacing:0.05em; text-transform:uppercase;
      padding:7px 13px; border-radius:2px; border:1px solid var(--line-soft); color:var(--muted);
      cursor:pointer; background:transparent; transition:all .15s ease;
    }
    .ficha-tab:hover{ color:var(--text); border-color:var(--cyan-dim); }
    .ficha-tab.active{ color:var(--cyan); border-color:var(--cyan); background:rgba(79,242,224,0.08); }

    .form-grid-2{ display:grid; grid-template-columns:1fr 1fr; gap:10px; }
    @media (max-width:520px){ .form-grid-2{ grid-template-columns:1fr; } }

    .entry-card{ background:var(--panel-alt); border:1px solid var(--line-soft); border-radius:3px; padding:12px 14px; margin-bottom:8px; }
    .entry-card-head{ display:flex; justify-content:space-between; align-items:flex-start; gap:8px; margin-bottom:6px; }
    .entry-date{ font-family:'JetBrains Mono',monospace; font-size:9.5px; color:var(--cyan-dim); white-space:nowrap; }
    .entry-text{ font-size:0.83rem; color:var(--text); line-height:1.5; white-space:pre-wrap; }
    .entry-del{ background:none; border:none; color:var(--muted); cursor:pointer; padding:2px; flex-shrink:0; }
    .entry-del:hover{ color:var(--magenta); }

    .form-item{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px 0; border-bottom:1px solid var(--line-soft); }
    .form-item:last-child{ border-bottom:none; }
    .form-item-name{ font-size:0.85rem; }
    .form-item-date{ font-family:'JetBrains Mono',monospace; font-size:9.5px; color:var(--muted); margin-top:2px; }
    .form-check{
      display:flex; align-items:center; gap:6px; cursor:pointer; font-family:'JetBrains Mono',monospace; font-size:9.5px;
      text-transform:uppercase; letter-spacing:0.05em; padding:5px 10px; border-radius:2px; border:1px solid var(--line-soft); background:transparent; color:var(--muted);
    }
    .form-check.done{ color:var(--cyan); border-color:var(--cyan-dim); background:rgba(79,242,224,0.06); }
...
function PaginaProntuario({
  pacientes, selecionadoId, onSelecionar, onAdicionar, onExcluir, onEditar,
  onNavegar, onSalvarDados, onAdicionarOrientacao, onRemoverOrientacao,
  onAdicionarAvaliacao, onRemoverAvaliacao, onAdicionarFormulario, onAlternarFormulario,
}) {
  ...
  const [edDiagnostico, setEdDiagnostico] = useState(Object.keys(DIAGNOSTICOS)[0]);
  const [edFase, setEdFase] = useState("Aguda");
  const [edPlanoNome, setEdPlanoNome] = useState("");

  const [abaFicha, setAbaFicha] = useState("painel");
  const [dDados, setDDados] = useState({
    email: "", telefone: "", cpf: "", dataNascimento: "",
  ...
  useEffect(() => {
    if (p) {
      setEdNome(p.nome);
      setEdDiagnostico(p.diagnostico);
      setEdFase(p.fase);
      setEdPlanoNome(p.plano.nome);
      setEditando(false);
      setAbaFicha("painel");
      const d = p.dados || {};
      setDDados({
        email: d.email || "", telefone: d.telefone || "", cpf: d.cpf || "",
        dataNascimento: d.dataNascimento || "", cep: d.cep || "", rua: d.rua || "",
        numero: d.numero || "", bairro: d.bairro || "", cidade: d.cidade || "", estado: d.estado || "",
      });
      setNovaOrientacao("");
      setNovaAvaliacao("");
      setNovoFormulario("");
        ...
        <div className="amp-panel">
          ...
          <div className="ficha-tabs">
            {[
              ...
              <button
                key={id}
                className={`ficha-tab ${abaFicha === id ? "active" : ""}`}
            ...
            <>
              <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 4 }}>
                <button className="btn ghost" onClick={() => setEditando((v) => !v)}>
                  {editando ? <X size={13} /> : null} {editando ? "Cancelar" : "Editar"}
                </button>
              </div>
              ...
              {editando ? (
                <div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid var(--line-soft)" }}>
                  <label className="field">Nome do paciente</label>
                  <input type="text" value={edNome} onChange={(e) => setEdNome(e.target.value)} style={{ marginBottom: 10 }} />

                  <label className="field">Diagnóstico</label>
                  <select value={edDiagnostico} onChange={(e) => setEdDiagnostico(e.target.value)} style={{ marginBottom: 10 }}>
                    {Object.keys(DIAGNOSTICOS).map((d) => <option key={d} value={d}>{d}</option>)}
                  </select>

                  <label className="field">Fase</label>
                  <select value={edFase} onChange={(e) => setEdFase(e.target.value)} style={{ marginBottom: 10 }}>
                    {FASES_OPCOES.map((f) => <option key={f} value={f}>{f}</option>)}
                  </select>

                  <label className="field">Plano / pacote</label>
                  <input type="text" value={edPlanoNome} onChange={(e) => setEdPlanoNome(e.target.value)} style={{ marginBottom: 12 }} />
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      onEditar(p.id, {
                        nome: edNome.trim(),
                        diagnostico: edDiagnostico,
                        fase: edFase,
                        planoNome: edPlanoNome.trim() || "Particular",
                      });
                      setEditando(false);
                    }}
                  >
          ...
          {abaFicha === "dados" && (
            <div>
              <div className="amp-eyebrow">Informações pessoais</div>
              <div className="form-grid-2" style={{ marginBottom: 10 }}>
                <div>
                  <label className="field">E-mail</label>
                  <input type="text" value={dDados.email} onChange={(e) => setDDados((d) => ({ ...d, email: e.target.value }))} placeholder="ex: ana@email.com" />
                </div>
                <div>
                  <label className="field">Telefone</label>
                  <input type="text" value={dDados.telefone} onChange={(e) => setDDados((d) => ({ ...d, telefone: e.target.value }))} placeholder="(48) 90000-0000" />
                </div>
                <div>
                  <label className="field">CPF</label>
                  <input type="text" value={dDados.cpf} onChange={(e) => setDDados((d) => ({ ...d, cpf: e.target.value }))} placeholder="000.000.000-00" />
                </div>
                <div>
                  <label className="field">Data de nascimento</label>
                  <input type="date" value={dDados.dataNascimento} onChange={(e) => setDDados((d) => ({ ...d, dataNascimento: e.target.value }))} />
                </div>
              </div>
              {dDados.dataNascimento && (
                <div className="row" style={{ marginBottom: 6 }}>
                  <span className="row-label">Idade</span>
                  <span className="mono">{calcularIdade(dDados.dataNascimento)} anos</span>
                </div>
              )}

              <div className="amp-eyebrow" style={{ marginTop: 16 }}>Endereço</div>
              <div className="form-grid-2" style={{ marginBottom: 12 }}>
                <div>
                  <label className="field">CEP</label>
                  <input type="text" value={dDados.cep} onChange={(e) => setDDados((d) => ({ ...d, cep: e.target.value }))} placeholder="00000-000" />
                </div>
                <div>
                  <label className="field">Rua</label>
                  <input type="text" value={dDados.rua} onChange={(e) => setDDados((d) => ({ ...d, rua: e.target.value }))} />
                </div>
                <div>
                  <label className="field">Número</label>
                  <input type="text" value={dDados.numero} onChange={(e) => setDDados((d) => ({ ...d, numero: e.target.value }))} />
                </div>
                <div>
                  <label className="field">Bairro</label>
                  <input type="text" value={dDados.bairro} onChange={(e) => setDDados((d) => ({ ...d, bairro: e.target.value }))} />
                </div>
                <div>
                  <label className="field">Cidade</label>
                  <input type="text" value={dDados.cidade} onChange={(e) => setDDados((d) => ({ ...d, cidade: e.target.value }))} />
                </div>
                <div>
                  <label className="field">Estado</label>
                  <input type="text" value={dDados.estado} onChange={(e) => setDDados((d) => ({ ...d, estado: e.target.value }))} placeholder="SC" />
                </div>
              </div>

              <button className="btn solid" style={{ width: "100%", justifyContent: "center" }} onClick={() => onSalvarDados(p.id, dDados)}>
                <Check size={13} /> Salvar dados
              </button>
            </div>
          )}

          {/* ---------- ORIENTAÇÕES ---------- */}
          {abaFicha === "orientacoes" && (
            <div>
              <label className="field">Nova orientação</label>
              <textarea
                value={novaOrientacao}
                onChange={(e) => setNovaOrientacao(e.target.value)}
                placeholder="ex: Evitar carga axial nos próximos 7 dias, aplicar gelo 3x ao dia..."
                rows={3}
                style={{
                  width: "100%", background: "var(--bg-alt)", border: "1px solid var(--line-soft)", color: "var(--text)",
                  fontFamily: "JetBrains Mono, monospace", fontSize: 12, padding: "8px 10px", borderRadius: 2, resize: "vertical", marginBottom: 8,
                }}
              />
              <button
                className="btn solid"
                ...
                onClick={() => {
                  ...
                  onAdicionarOrientacao(p.id, novaOrientacao.trim());
                  setNovaOrientacao("");
                }}
              >
                <Plus size={13} /> Adicionar orientação
              </button>

              {(p.orientacoes || []).length === 0 ? (
                <div className="empty">Nenhuma orientação registrada.</div>
              ) : (
                (p.orientacoes || []).map((o) => (
                  <div className="entry-card" key={o.id}>
                    <div className="entry-card-head">
                      <span className="entry-date">{formatarData(o.data)}</span>
                      <button className="entry-del" onClick={() => onRemoverOrientacao(p.id, o.id)} title="Remover">
                        <X size={13} />
                      </button>
                    </div>
                    <div className="entry-text">{o.texto}</div>
                  </div>
                ))
              )}
            </div>
          ...
          {abaFicha === "avaliacao" && (
            <div>
              <label className="field">Registrar avaliação clínica</label>
              <textarea
                value={novaAvaliacao}
                onChange={(e) => setNovaAvaliacao(e.target.value)}
                placeholder="ex: Testes especiais, achados de exame físico, evolução do quadro..."
                rows={3}
                style={{
                  width: "100%", background: "var(--bg-alt)", border: "1px solid var(--line-soft)", color: "var(--text)",
                  fontFamily: "JetBrains Mono, monospace", fontSize: 12, padding: "8px 10px", borderRadius: 2, resize: "vertical", marginBottom: 8,
              ...
              <button
                ...
                onClick={() => {
                  ...
                  onAdicionarAvaliacao(p.id, novaAvaliacao.trim());
                  setNovaAvaliacao("");
                }}
              >
                <Plus size={13} /> Registrar avaliação
              </button>
              ...
              {(p.avaliacoes || []).length === 0 ? (
                ...
                (p.avaliacoes || []).map((a) => (
                  <div className="entry-card" key={a.id}>
                    <div className="entry-card-head">
                      <span className="entry-date">{formatarData(a.data)}</span>
                      <button className="entry-del" onClick={() => onRemoverAvaliacao(p.id, a.id)} title="Remover">
                        <X size={13} />
                      </button>
                    </div>
                    <div className="entry-text">{a.texto}</div>
                  </div>
                ))
              )}
            </div>
          ...
          {abaFicha === "sessoes" && (
            <div>
              {p.sessoes.length === 0 ? (
                <div className="empty">Nenhuma sessão registrada.</div>
              ) : (
                [...p.sessoes].reverse().map((s) => (
                  <div className="form-item" key={s.s}>
                    <div>
                      <div className="form-item-name">Sessão {s.s}</div>
                      <div className="form-item-date mono">{DIAGNOSTICOS[p.diagnostico].escala}: {s.escore} · ADM {s.adm}° · Dor {s.dor}/10</div>
                    </div>
                  </div>
                ))
              )}
              <button className="btn ghost" style={{ width: "100%", justifyContent: "center", marginTop: 12 }} onClick={() => onNavegar("evolucao")}>
                <Activity size={13} /> Ver painel de evolução completo
              </button>
            </div>
          )}

          {/* ---------- FORMULÁRIOS ---------- */}
          {abaFicha === "formularios" && (
            <div>
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                <input
                  type="text"
                  value={novoFormulario}
                  onChange={(e) => setNovoFormulario(e.target.value)}
                  placeholder={`ex: ${DIAGNOSTICOS[p.diagnostico].escala}`}
                />
                <button
                  className="btn solid"
                  style={{ whiteSpace: "nowrap" }}
                  onClick={() => {
                    const nomeForm = novoFormulario.trim() || DIAGNOSTICOS[p.diagnostico].escala;
                    onAdicionarFormulario(p.id, nomeForm);
                    setNovoFormulario("");
                  }}
                >
              ...
              {(p.formularios || []).length === 0 ? (
                <div className="empty">Nenhum formulário aplicado ainda.</div>
              ) : (
                (p.formularios || []).map((f) => (
                  <div className="form-item" key={f.id}>
                    <div>
                      <div className="form-item-name">{f.nome}</div>
                      <div className="form-item-date mono">{f.respondido ? `respondido em ${formatarData(f.data)}` : "pendente"}</div>
                    </div>
                    <button
                      className={`form-check ${f.respondido ? "done" : ""}`}
                      onClick={() => onAlternarFormulario(p.id, f.id)}
                    >
                      {f.respondido ? <Check size={11} /> : null} {f.respondido ? "Respondido" : "Marcar respondido"}
                    </button>
                  </div>
                ))
              )}
...
export default function Amplitude() {
  ...
  const adicionarPaciente = ({ nome, diagnostico, fase, plano, sessaoInicial }) => {
    const novo = {
      ...
      dados: { email: "", telefone: "", cpf: "", dataNascimento: "", cep: "", rua: "", numero: "", bairro: "", cidade: "", estado: "" },
      orientacoes: [],
      avaliacoes: [],
      formularios: [{ id: `f${Date.now()}`, nome: DIAGNOSTICOS[diagnostico].escala, respondido: true, data: new Date().toISOString() }],
    };
    ...
    setPacientes((prev) =>
      prev.map((p) => {
        ...
        return {
          ...
          fase: dados.fase,
          plano: { ...p.plano, nome: dados.planoNome },
        };
      })
    );
  };

  const atualizarPaciente = (id, updater) => {
    setPacientes((prev) => prev.map((p) => (p.id === id ? updater(p) : p)));
  };

  const salvarDadosPessoais = (id, dados) => atualizarPaciente(id, (p) => ({ ...p, dados }));

  const adicionarOrientacao = (id, texto) => atualizarPaciente(id, (p) => ({
    ...p,
    orientacoes: [{ id: `o${Date.now()}`, texto, data: new Date().toISOString() }, ...(p.orientacoes || [])],
  }));

  const removerOrientacao = (id, orientacaoId) => atualizarPaciente(id, (p) => ({
    ...p,
    orientacoes: (p.orientacoes || []).filter((o) => o.id !== orientacaoId),
  }));

  const adicionarAvaliacao = (id, texto) => atualizarPaciente(id, (p) => ({
    ...p,
    avaliacoes: [{ id: `a${Date.now()}`, texto, data: new Date().toISOString() }, ...(p.avaliacoes || [])],
  }));

  const removerAvaliacao = (id, avaliacaoId) => atualizarPaciente(id, (p) => ({
    ...p,
    avaliacoes: (p.avaliacoes || []).filter((a) => a.id !== avaliacaoId),
  }));

  const adicionarFormulario = (id, nome) => atualizarPaciente(id, (p) => ({
    ...p,
    formularios: [...(p.formularios || []), { id: `f${Date.now()}`, nome, respondido: false, data: null }],
  }));

  const alternarFormulario = (id, formularioId) => atualizarPaciente(id, (p) => ({
    ...p,
    formularios: (p.formularios || []).map((f) =>
      f.id === formularioId ? { ...f, respondido: !f.respondido, data: !f.respondido ? new Date().toISOString() : null } : f
    ),
  }));
    ...
    <div className="amp-shell">
      ...
      <main className="amp-main">
        ...
        <div className="amp-content">
          ...
          {!carregando && pagina === "pacientes" && (
            <PaginaProntuario
              pacientes={pacientes}
              selecionadoId={selecionadoId}
              onSelecionar={setSelecionadoId}
              onAdicionar={adicionarPaciente}
              onExcluir={excluirPaciente}
              onEditar={editarPaciente}
              onNavegar={setPagina}
              onSalvarDados={salvarDadosPessoais}
              onAdicionarOrientacao={adicionarOrientacao}
              onRemoverOrientacao={removerOrientacao}
              onAdicionarAvaliacao={adicionarAvaliacao}
              onRemoverAvaliacao={removerAvaliacao}
              onAdicionarFormulario={adicionarFormulario}
              onAlternarFormulario={alternarFormulario}
            />
          )}
